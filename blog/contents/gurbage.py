...
#
# *. We can read 'abba' if we reverse the letters, it’s still 'abba', so this string is called palindrome.
...
#
#
# *. Answer without running the code, run in your brain.
...
#
#
# https://python.plainenglish.io/how-to-make-a-contact-book-application-using-python-9db4f8dfa4fe
# *. Write a program to display a simple form of digital book. “Books” are text files in which each block (page) of
# text is followed by a double dash (--).
...
# If the number is positive, the given number of pages are skipped. If there is no number, the next page is displayed.
#
...
# Readme.txt, for example, appears many times on our hard disks.
# These files are distinct and normally have different contents.
# How is it that the operating system can maintain separate files of the same name?
#
#
#
